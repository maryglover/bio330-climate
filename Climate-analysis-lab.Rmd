...
R can perform basic mathematical calculations. To compute a calculations, simply type it in the *Console*. You can do addition (+), subtraction (-), multiplication (*), division (/), power (^), to name a few. You can also use parentheses as usual in mathematic equations.
...
Functions are run using the function name followed by the "arguements." A function can have any number of arguments.

![Functions](function.png){width=50%}

Functions can range from very simple to very complex. Many functions exist in R without doing anything, you can "load" additional functions thorugh "packages", or you can even make your own functions. 
...
Table==, sum, max, min, mean, median, sd
...
So far, we have only dealt with single numbers, but R can handle numbers or text.  With numbers, you can type them in reguarly and R will recognize them as such. Text is put into quotations (""). 
...
- scale_y_continuous to change the y axis names
- scale_y_continuous to change the break points and names on the x axis
